pub mod animation_message;
pub mod audio_message;
pub mod boost_added_message;
pub mod channel_chat_created_message;
pub mod chat_background_set_message;
pub mod chat_shared_message;
pub mod command_message;
pub mod connected_website_message;
pub mod contract_message;
pub mod delete_chat_photo_message;
pub mod dice;
pub mod document_message;
pub mod forum_topic_closed_message;
pub mod forum_topic_created_message;
pub mod forum_topic_edited_message;
pub mod forum_topic_reopened_message;
pub mod game_message;
pub mod general_forum_topic_hidden_message;
pub mod general_forum_topic_unhidden_message;
pub mod giveaway_completed_message;
pub mod giveaway_created_message;
pub mod giveaway_message;
pub mod giveaway_winners_message;
pub mod group_chat_created_message;
pub mod inaccessible_message;
pub mod invoice_message;
pub mod left_chat_membe_message;
pub mod location;
pub mod message_auto_delete_timer_changed_message;
pub mod migrate_from_chat_message;
pub mod migrate_to_chat_message;
pub mod new_chat_members_message;
pub mod new_chat_photo_message;
pub mod new_chat_title_message;
pub mod passport_data_message;
pub mod photo_message;
pub mod pinned_message;
pub mod poll;
pub mod proximity_alert_triggered_message;
pub mod sticker_message;
pub mod story_message;
pub mod successful_payment_message;
pub mod supergroup_chat_created_message;
pub mod text_message;
pub mod users_shared_message;
pub mod venue_message;
pub mod video_chat_ended_message;
pub mod video_chat_participants_invited_message;
pub mod video_chat_scheduled_message;
pub mod video_chat_started_message;
pub mod video_message;
pub mod video_note_message;
pub mod voice_message;
pub mod web_app_data_message;
pub mod write_access_allowed_message;
